...
The commands below reflect the installed CLI. Do not run `herdr --help` or a bare command group (e.g. `herdr agent`) for discovery before starting work — only fall back to it if a documented command errors or behaves unexpectedly, per the version-skew note in Safety and coordination rules. Do not run bare `herdr` for discovery; it launches or attaches the TUI. Do not probe a mutating nested command by omitting arguments. Commands such as `herdr workspace create` are valid with defaults and will execute.
...
Create a sibling pane as above, then run and inspect the command:
...
- If a documented command errors or behaves unexpectedly, run `herdr <group> --help` (or bare `herdr <group>`) to confirm current syntax before retrying — do not repeat this speculatively for commands that already worked.
