...
1. **Title and Badges**: `# <Project Name>`. Use Shields.io badges with `style=for-the-badge`, one badge per line, grouped into logical rows. Put project/platform badges first, then language/framework/tool badges. Include only relevant badges such as license, build status, runtime, framework, and major integrations. Do not use small/default badge styling when the project has an established badge style.
...
For generic projects, add a short blockquote summary below the badge rows when
it improves discoverability. Example:

```markdown
> A command-line tool that downloads and organizes course materials.
```
