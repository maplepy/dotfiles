...
Singleton {
    ...
    function toPlainObject(qtObj) {
        ...
        if (Array.isArray(qtObj))
            return qtObj.map((item) => {
                return toPlainObject(item);
            });

        // Handle array-like Qt objects (e.g., have length and numeric keys)
        if (typeof qtObj.length === "number" && qtObj.length > 0 && Object.keys(qtObj).every((key) => {
    ...
    function applyToQtObject(qtObj, jsonObj) {
        ...
        const isQtArrayLike = (obj) => {
            return obj && typeof obj === "object" && typeof obj.length === "number" && obj.length > 0 && Object.keys(obj).every((key) => {
                return !isNaN(key) || key === "length";
            });
        };
