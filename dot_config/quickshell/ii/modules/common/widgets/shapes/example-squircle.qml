...
Window {
    ...
    RowLayout {
        ...
        ShapeCanvas {
            // bottom-right
            // We specify the point...
            // ...and the rounding. The 1st value is size of the rounding, 2nd is smoothing
            // bottom-left
            // top-left
            // top-right
            // Make sure it's properly centered and scaled to perfectly fit [0, 1]
            ...
            roundedPolygon: MaterialShapes.customPolygon([new MaterialShapes.PointNRound(new Offset.Offset(1, 0.67), new CornerRounding.CornerRounding(0.211, 2)), new MaterialShapes.PointNRound(new Offset.Offset(0, 0.67), new CornerRounding.CornerRounding(0.207, 2)), new MaterialShapes.PointNRound(new Offset.Offset(0, 0), new CornerRounding.CornerRounding(0.207, 2)), new MaterialShapes.PointNRound(new Offset.Offset(1, 0), new CornerRounding.CornerRounding(0.211, 2))], 1).normalized()
        }
